...
export const ActionFreeze: React.FC<{
  ...
  const handleSend = async () => {
    const addressHex = new Address(addressTarget).toHex();

    let payload = `freeze@${tokenIdentifierHex}@${addressHex}`;
    if (nonce.isGreaterThan(0)) {
      payload = `freezeSingleNFT@${tokenIdentifierHex}@${bigNumToHex(
        nonce
      )}@${addressHex}`;
    }
