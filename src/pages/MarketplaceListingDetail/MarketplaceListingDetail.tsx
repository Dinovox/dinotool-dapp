...
function useCountdown(endTime?: number) {
  ...
  /*
   * If endTime is provided, returns formatted time left.
   * If now < endTime, it is standard countdown.
   * If now > endTime, returns 0s.
   * Logic for "Starts in" vs "Ends in" relies on parent passing correct target.
   */
  if (!endTime) return '—';
  if (endTime > 32503680000000) return t('marketplace:infinite');

  // If the target is in the past, return 0
  if (now >= endTime) return '0d 0h 0m 0s';
...
export const MarketplaceListingDetail = () => {
  ...
  const listing: Listing | null = useMemo(() => {
    ...
    const listingData: Listing = {
      id: id,
      auction_type: rawAuction.auction_type,
      ...
      auction: {
        ...
        maxPerWallet: rawAuction.max_per_wallet
          ? new BigNumber(rawAuction.max_per_wallet.toString())
          : undefined
      },
  ...
  // 4.1 Fetch User Purchased Amount (for Max Per Wallet check) - AFTER listing is defined
  const { purchasedAmount } = useGetUserPurchasedAmount(
    rawAuction?.auction_id?.toString() || id,
    address
  );

  // Calculate remaining allowed for this user
  const remainingAllowed = useMemo(() => {
    if (!listing?.auction?.maxPerWallet) return null; // No limit
    const limit = listing.auction.maxPerWallet;
    if (limit.lte(0)) return null;
    const purchased = purchasedAmount || new BigNumber(0);
    return limit.minus(purchased);
  ...
  const hasStarted = Date.now() >= (listing?.auction?.startTime || 0);

  const timeLeft = useCountdown(
    hasStarted ? listing?.auction?.endTime : listing?.auction?.startTime
  );
            ...
            <Card>
              ...
              <CardContent className='space-y-4'>
                ...
                <div className='rounded-xl border p-4'>
                  ...
                  {listing.auction?.auctionType?.name !== 'SftOnePerPayment' &&
                    ...
                    new BigNumber(
                      ...
                      <div className='mt-2 flex flex-col gap-1 border-t border-slate-100 pt-2 text-sm text-slate-500'>
                        <div className='flex justify-between items-center'>
                          <span>{t('marketplace:qty')}:</span>
                          <span className='font-medium text-slate-900'>
                            {listing.auction.auctionedTokens.amount.toString()}
                          </span>
                        </div>
                        <div className='flex justify-between items-center'>
                          <span>{t('marketplace:unit_price')}:</span>
                          <span className='font-medium text-slate-900'>
                            <FormatAmount
                              amount={(
                                (isAuction && !isDirectSale
                  ...
                  {isAuction && (
                    <div className='mt-3 flex items-center gap-2 text-sm'>
                      <Badge>
                        {hasStarted
                          ? t('marketplace:ends_in')
                          : t('marketplace:starts_in')}
                      </Badge>
                  ...
                  <>
                    {isAuction ? (
                      <div className='space-y-3'>
                              ...
                              {address === listing.auction?.currentWinner ? (
                                ...
                                <>
                                  <input
                                    ...
                                    inputMode='decimal'
                                    className='h-10 flex-1 rounded-md border border-gray-300 bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-slate-400 disabled:opacity-50 disabled:bg-slate-50'
                                  />
                        ...
                        {!hasStarted && (
                          <div className='rounded-md bg-amber-50 p-3 text-sm text-center text-amber-800 border border-amber-200 font-medium'>
                            {t('marketplace:auction_not_started')}
                          </div>
                        )}
                            ...
                            <ActionBuySft
                              ...
                              disabled={
                                ...
                                (remainingAllowed !== null &&
                                  new BigNumber(qty).gt(remainingAllowed))
                              }
                            />
                            {!hasStarted && (
                              <div className='mt-2 rounded-md bg-amber-50 p-3 text-sm text-center text-amber-800 border border-amber-200 font-medium'>
                                {t('marketplace:auction_not_started')}
                              </div>
                            )}
                            {isLimitReached && (
                              <div className='mt-2 w-full rounded-md bg-red-50 p-2 text-xs text-center text-red-600 border border-red-200'>
                                {t
                                  ? t('marketplace:max_per_wallet_reached')
                                  : 'Purchase limit reached for this wallet.'}
                              </div>
                            )}
                            {!isLimitReached &&
                              remainingAllowed !== null &&
                              remainingAllowed.gt(0) && (
                                <div className='mt-2 w-full rounded-md bg-amber-50 p-2 text-xs text-center text-amber-800 border border-amber-200'>
                                  {t('marketplace:remaining_allowed')}:{' '}
                                  {remainingAllowed.toString()}
                                </div>
                              )}
                ...
                <div className='grid grid-cols-2 gap-3 pt-2 text-sm'>
                  ...
                  <div className='rounded-md border p-3'>
                    <div className='text-xs text-slate-500'>
                      {t ? t('marketplace:start_time') : 'Start Date'}
                    </div>
                    <div className='font-medium'>
                      {new Date(
                        listing.auction?.startTime || 0
                      ).toLocaleString()}
                    </div>
                  </div>
                  {isAuction && (
