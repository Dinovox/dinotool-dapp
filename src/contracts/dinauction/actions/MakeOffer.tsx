...
export interface ActionMakeOfferProps {
  nftIdentifier: string;
  nftNonce: string | number;
  nftAmount?: number; // Quantity of NFT wanted, usually 1
...
export const ActionMakeOffer = ({
  ...
  const monitorTransaction = async (hash: string) => {
    try {
      console.log('Monitoring MakeOffer transaction:', hash);
      await waitForTransactionEvent(hash, MARKETPLACE_EVENTS);
      console.log('MakeOffer confirmed, syncing...');
      await callSyncMarketplaceActivity();
      onSuccess?.();
      setTransactionSessionId(null);
    } catch (err) {
      console.error('Error monitoring MakeOffer transaction:', err);
      setTransactionSessionId(null);
    }
  };

  const sendMakeOfferTransaction = async () => {
    ...
    // Args: desired_nft_id, desired_nft_nonce, desired_amount, deadline, opt_auction_id
    // Note: desired_amount needs to be passed.
    ...
    if (isEGLD) {
      // EGLD: sendOffer@args
      const payload = 'sendOffer' + argsPayload;
      ...
      // ESDT: ESDTTransfer@Token@Amount@sendOffer@args
      // Note: ESDTTransfer to Contract
      const payload =
        'ESDTTransfer@' +
        ...
        '@' +
        Buffer.from('sendOffer', 'utf8').toString('hex') +
        argsPayload;
    ...
    const sessionId = await signAndSendTransactions({
      ...
      transactionsDisplayInfo: {
        processingMessage: 'Processing offer transaction',
        errorMessage: 'An error occurred while making the offer',
        successMessage: 'Offer made successfully'
      }
    });
  ...
  return (
    <>
      {!hasPendingTransactions ? (
        <button
          onClick={sendMakeOfferTransaction}
          disabled={disabled}
